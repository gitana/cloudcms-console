(function($) {
    Gitana.Console.Pages.GitanaWebHostSelector = Gitana.CMS.Pages.AbstractListPageGadget.extend(
    {
        TEMPLATE : "components/gitana-webhost-selector",

        SUBSCRIPTION : "webhost-picker",

        DISPLAY_LIST_FILTER: true,

        FILTER : "webhost-candidates-list-filters",

        FILTER_TOOLBAR: {
            "query" : {
                "title" : "Query Web Host",
        ...
        setupList: function(el) {
            ...
            list["columns"] = [
                ...
                },
                {
                    ...
                    "property": function(callback) {
                        ...
                        var value = "<a class='gitana-selector-action webhost-action gitana-selector-select webhost-select' data-target-webhost-id='" + id + "' data-target-webhost-title='" + title + "'><span>Select</span></a>";
                        callback(value);
                    }
                }
            ];
            ...
            list["loadFunction"] = function(query, pagination, callback) {
                ...
                Chain(self.platform()).queryWebHosts(_query, self.pagination(pagination)).then(function() {
                    callback.call(this);
                });
            };
        ...
        processList: function(el) {
            ...
            $("body").undelegate(".webhost-select", "click").delegate(".webhost-select", "click", function() {

                var control = $(this);
                var id = control.attr("data-target-webhost-id");
                var title = control.attr("data-target-webhost-title");

                self.selectHandler.call(this, id, title);
            })
        },

        setupGitanaWebHostSelector : function(el) {

            var page = {
                "title" : "Web Host List",
                "description" : "List of Web Hosts",
            ...
            this.observable('webhostselector').set(page);
        },
            ...
            self.loadContext(el, function() {
                ...
                self.checkAuthorities(function(isEntitled, error) {
                    if (isEntitled) {
                        ...
                        // set up the page
                        self.setupGitanaWebHostSelector(el);
    ...
    Ratchet.GadgetRegistry.register("webhostselector", Gitana.Console.Pages.GitanaWebHostSelector);
