(function($) {
    Gitana.Console.Pages.GitanaAuthGrantSelector = Gitana.CMS.Pages.AbstractListPageGadget.extend(
    {
        TEMPLATE : "components/gitana-authgrant-selector",

        SUBSCRIPTION : "authgrant-picker",

        DISPLAY_LIST_FILTER: true,

        FILTER : "authgrant-candidates-list-filters",

        FILTER_TOOLBAR: {
            "query" : {
                "title" : "Query Auth Grant",
        ...
        setupList: function(el) {
            ...
            list["columns"] = [
                ...
                },
                {
                    ...
                    "property": function(callback) {
                        ...
                        var value = "<a class='gitana-selector-action authgrant-action gitana-selector-select authgrant-select' data-target-authgrant-id='" + id + "' data-target-authgrant-title='" + title + "'><span>Select</span></a>";
                        callback(value);
                    }
                }
            ];
            ...
            list["loadFunction"] = function(query, pagination, callback) {
                ...
                Chain(self.platform()).queryAuthenticationGrants(_query, self.pagination(pagination)).then(function() {
                    callback.call(this);
                });
            };
        ...
        processList: function(el) {
            ...
            $("body").undelegate(".authgrant-select", "click").delegate(".authgrant-select", "click", function() {

                var control = $(this);
                var id = control.attr("data-target-authgrant-id");
                var title = control.attr("data-target-authgrant-title");

                self.selectHandler.call(this, id, title);
            })
        },

        setupGitanaAuthGrantSelector : function(el) {

            var page = {
                "title" : "Auth Grant List",
                "description" : "List of Auth Grants",
                "listTitle" : "Auth Grant List",
                "listIcon" : Gitana.Utils.Image.buildImageUri('objects', 'authentication-grant', 20),
                "subscription" : this.SUBSCRIPTION,
                "filter" : this.FILTER
            };

            this.observable('authgrantselector').set(page);
        },
            ...
            self.loadContext(el, function() {
                ...
                self.checkAuthorities(function(isEntitled, error) {
                    if (isEntitled) {
                        ...
                        // set up the page
                        self.setupGitanaAuthGrantSelector(el);
    ...
    Ratchet.GadgetRegistry.register("authgrantselector", Gitana.Console.Pages.GitanaAuthGrantSelector);
